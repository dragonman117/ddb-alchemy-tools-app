...
export async function bulkMonsterFetch(skip: number, take: number, search: string, urlTool: UrlTools): Promise<any | null> {
  const [monsterUrl, monsterHeader] = urlTool.getMonsterSearchUrl(skip, take, search)
  // eslint-disable-next-line no-undef
  const res = await fetch(new URL(monsterUrl as string), { method: 'GET', headers: monsterHeader as HeadersInit })
