  ...
  <div class="flex flex-row place-content-center grow bg-stone-300">
    <div class="flex flex-col grow m-2 bg-stone-50 drop-shadow-lg">
      ...
      <div class="flex flex-col overflow-auto altHeight">
        ...
        <monster-item
          v-for="monster in searchedMonsters"
          :key="monster.name"
          :monster="monster"
          :show-add="true"
          @add="addHit(monster)"
      ...
      <div
        class="flex flex-row grow bg-stone-700 p-2 place-content-center content-center justify-center"
      >
        <div
          class="stone-button p-1 px-3 mx-2 z-10"
        ...
        <div
          class="stone-button p-1 px-5 mx-2 w-10 z-10"
          v-if="current_page - 2 > 0"
          @click="loadPage(current_page - 2)"
        >
          {{ current_page - 2 }}
        </div>
        <div
        ...
        <div
          class="px-5 mx-2 w-4 w-10 flex flex-row place-content-center content-center items-center text-white"
          v-if="current_page > 0"
        >
          {{ current_page }}
        </div>
        <div
          class="stone-button p-1 px-5 mx-2 w-10 z-10"
          v-if="current_page + 1 <= total_pages"
          @click="loadPage(current_page + 1)"
        >
          {{ current_page + 1 }}
        </div>
        <div
          class="stone-button p-1 px-5 mx-2 w-10 z-10"
          v-if="current_page + 2 <= total_pages"
          @click="loadPage(current_page - 2)"
        >
          {{ current_page + 2 }}
        </div>
    ...
    <div class="flex flex-col grow m-2 bg-stone-50 drop-shadow-lg max-w-md max-h-screen">
      ...
      <div
        class="flex flex-col overflow-auto add-content"
        :class="{ altHeight: url_import_visible }"
      >
        <div
          class="flex flex-col absolute w-full h-full z-30 add-content bg-stone-50 opacity-70 place-content-center"
          :class="{ altHeight: url_import_visible }"
          v-if="generating"
        >
        ...
        <monster-item
          v-for="(monster, index) in selectedMonsters"
          :key="monster.name"
          :monster="monster"
          @remove="removeHit(index)"
        />
      </div>
      ...
      <div class="flex flex-row bg-stone-700 p-2 place-content-center">
        <div
          class="stone-button has-tooltip z-10"
          v-show="!url_import_visible"
          @click="showUrlImport()"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            class="tooltip bottom-12 bg-stone-800 p-2 w-24 text-stone-50 text-xs flex place-content-center rounded-md"
            >Add By Url</div
          >
        </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    async loadPage(pg: number) {
      this.searchedMonsters = []
      this.loading = true
      const res = await window.mainApi.send('monsterSearch', [
        this.search_url,
        (pg - 1) * 50,
        50,
        true
